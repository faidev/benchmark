...
sb_test() {
    ...
    table_size=1000000
    if [ "$3" == "read-only" ];then oltp_script='oltp_read_only.lua';else oltp_script='oltp_read_write.lua';fi    #根据脚本参数确定是否read-only
...
EOF
    ...
    for i in `seq $times`;do
        for sb_threds in $threds_num;do    #按照指定的sysbench线程测试
            ...
            load=$(uptime|awk -F: '{print$NF}'|awk -F, '{print$1}' 2>/dev/null)
            result=$(cat $log | egrep  "read:|write:|queries:|total:|total\ time:|.*95.*percentile:" | sed -r -e "s/[0-9]+ \(//g" -e "s/\ per sec\.\)//g" -e "s/m?s$//g"| awk  '{printf("%s ",$NF)}' | sed "s/\ /,/g" | sed "s/,$//g")
            ...
            mysql -u$m_user -p$m_passwd -P$m_port -h$m_host <<EOF
                ...
                VALUES ('$2','$m_host','$3','$sb_threds','$load',$result);
EOF
     
            if [ $? -ne 0 ];then
                echo -e "\n----------$sb_threds线程测试，第$i次插入数据库时失败----------"
                echo "INSERT VALUES ('$2','$m_host','$3',$sb_threds,$load,$result)"
                exit -2
            fi
